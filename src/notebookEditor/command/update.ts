...
export const applyDocumentUpdates = (editor: Editor, documentUpdates: AbstractDocumentUpdate[]): boolean => {
  const steps: Step[] = [];
  let finalSelection: Selection = editor.view.state.selection/*default*/;

  let currentState = editor.view.state/*default starting state*/;
  ...
  const finalTransaction = editor.view.state.tr/*new Transaction*/;
