...
// == Type ========================================================================
export type ProseMirrorInputRulePluginState = { transaction: Transaction; from: number; to: number; text: string; } | null;
type ProseMirrorInputRuleFunctionHandlerType = (state: EditorState, match: RegExpMatchArray, start: number, end: number) => Transaction | null;
 ...
 */
export class ProseMirrorInputRule {
  /**
   ...
   */
  public readonly handler: string | ProseMirrorInputRuleFunctionHandlerType;

  /** the {@link RegExp} that checks the typed Text */
  public readonly match: RegExp;

  constructor(match: RegExp, handler: string | ProseMirrorInputRuleFunctionHandlerType) {
    this.match = match;
 ...
 * */
 export const proseMirrorInputRulePlugin = ({ rules }: { rules: readonly ProseMirrorInputRule[]; }) => {
  const plugin: Plugin<ProseMirrorInputRulePluginState> = new Plugin<ProseMirrorInputRulePluginState>({
    // -- State -------------------------------------------------------------------
    state: {
 ...
 */
const executeInputRule = (view: EditorView, from: number, to: number, text: string, rules: readonly ProseMirrorInputRule[], plugin: Plugin) => {
