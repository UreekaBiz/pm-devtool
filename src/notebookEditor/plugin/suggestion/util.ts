...
import { escapeStringForRegEx } from 'common';
...
export const findSuggestionMatch = (config: TriggerConfig): SuggestionMatch => {
  ...
  const escapedChar = escapeStringForRegEx(char)/*create a regEx that matches the given char*/;
  const suffix = new RegExp(`\\s${escapedChar}$`);
  const prefix = startOfLine ? '^' : '';
  const resultingRegExp = allowSpaces
  ...
  const match = Array.from(text.matchAll(resultingRegExp)).pop()/*get last matched element*/;
  if(!match || match.input === undefined || match.index === undefined) return null/*not a valid match*/;

  // check that the first character is a space or the start of the line
  ...
  // if spaces are allowed and selection is directly between two triggers, account for it
